...
class Logger {
    ...
    verbose(...messages) {
        if (this.logLevel < 2 /* VERBOSE */)
            return;
        console.log(chalk__default["default"][this.VERBOSE_COLOR]("[VERBOSE]"), ...messages);
    }
    /**
    ...
    debug(...messages) {
        if (this.logLevel < 3 /* DEBUG */)
            return;
        console.log(chalk__default["default"][this.DEBUG_COLOR]("[DEBUG]"), ...messages);
    }
    /**
     * Logs warning-related messages
     */
    warn(...messages) {
        console.warn(chalk__default["default"][this.WARNING_COLOR](`(!)`, ...messages));
    }
}

const realReadonlyFileSystem = {
    statSync: fs__namespace.statSync,
    lstatSync: fs__namespace.lstatSync,
    readdirSync: fs__namespace.readdirSync,
...
function createTaskOptions({ typescript = TS__namespace, fileSystem = realReadonlyFileSystem, debug = false, cwd = process.cwd(), preserveModuleSpecifiers = "external", logger = new Logger(debug !== false ? 3 /* DEBUG */ : 0 /* NONE */) } = {}) {
    return {
        typescript,
        fileSystem,
        debug,
...
function createTransformTaskOptions({ fileSystem = realFileSystem, write = true, input, hooks = {}, outDir, ...rest }) {
    ...
    return {
        ...
        fileSystem,
        hooks,
        input: ensureArray(input).map(file => path__default["default"].normalize(path__default["default"].isAbsolute(file) ? file : path__default["default"].join(taskOptions.cwd, file))),
        outDir: path__default["default"].normalize(path__default["default"].isAbsolute(outDir) ? outDir : path__default["default"].join(taskOptions.cwd, outDir))
    };
}

const KNOWN_EXTENSIONS = [".d.ts", ".d.dts.map", ".js.map", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".mjsx", ".json", ".tsbuildinfo"];
/**
 ...
 */
function resolvePath({ id, parent, cwd, prioritizedPackageKeys = ["es2015", "esm2015", "module", "jsnext:main", "main", "browser"], prioritizedExtensions = ["", ".js", ".mjs", ".jsx", ".ts", ".tsx", ".json"], moduleDirectory = "node_modules", fileSystem, resolveCache }) {
    var _a, _b;
    id = path__default["default"].normalize(id);
    if (parent != null) {
        parent = path__default["default"].normalize(parent);
    }
    ...
    if (!isExternalLibrary(id)) {
        const absolute = path__default["default"].isAbsolute(id) ? path__default["default"].normalize(id) : path__default["default"].join(parent == null ? "" : path__default["default"].dirname(parent), id);
        const variants = [absolute, path__default["default"].join(absolute, "index")];
    ...
    try {
        const resolveResult = path__default["default"].normalize(resolve.sync(id, {
            basedir: path__default["default"].normalize(cwd),
...
function transformModuleSpecifier(moduleSpecifier, { context, parent, resolvedModuleSpecifier }) {
    ...
    if (path__default["default"].extname(moduleSpecifier) !== "" || resolvedModuleSpecifier == null) {
    ...
    return setExtension(ensureHasLeadingDotAndPosix(path__default["default"].relative(path__default["default"].dirname(parent), resolvedModuleSpecifier)), determineNewExtension(path__default["default"].extname(resolvedModuleSpecifier)));
}
...
function generateNameFromModuleSpecifier(moduleSpecifier) {
    const { name } = path__default["default"].parse(moduleSpecifier);
                ...
                // rather than generating a new unnecessary import
                    ...
                    context.addImport(factory.createImportDeclaration(undefined, undefined, moduleExports.hasDefaultExport
                        ...
                        : // Otherwise, import the entire namespace
                            factory.createImportClause(false, undefined, factory.createNamespaceImport(identifier)), factory.createStringLiteral(transformedModuleSpecifier)));
                }
        ...
        // import the default export (if it has any, otherwise import the entire namespace).
            ...
            // and then replace this CallExpression with an Object literal that can be destructured
                ...
                if (importSpecifiers.length > 0) {
                    context.addImport(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports(importSpecifiers)), factory.createStringLiteral(transformedModuleSpecifier)));
                }
    ...
    if (typescript.isIdentifier(node.name)) {
        ...
        // and it should preferably be converted into an ExportDeclaration
            ...
            // Otherwise, for older TypeScript versions, we'll have to first import and then re-export the namespace
                ...
                context.addImport(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamespaceImport(factory.createIdentifier(node.name.text))), moduleSpecifierExpression));
        ...
        // depending on whether or not the module has a default export
            ...
            context.addImport(factory.createImportDeclaration(undefined, undefined, moduleExports == null || moduleExports.hasDefaultExport
                ...
                : // Otherwise, import the entire namespace
                    factory.createImportClause(false, undefined, factory.createNamespaceImport(factory.createIdentifier(newName))), factory.createStringLiteral(transformedModuleSpecifier)));
            if (willReassign) {
    ...
    // We will only consider the simplest variants of these before opting out and letting the CallExpression visitor handle more sophisticated behavior
        ...
        if (importSpecifiers.length > 0) {
            ...
            for (const importSpecifier of importSpecifiersThatWillBeReassigned) {
                ...
                context.addImport(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, namedImports), factory.createStringLiteral(transformedModuleSpecifier)));
                // Now, immediately add a local mutable variable with the correct name
            ...
            if (otherImportSpecifiers.length > 0) {
                context.addImport(factory.createImportDeclaration(undefined, undefined, factory.createImportClause(false, undefined, factory.createNamedImports(otherImportSpecifiers)), factory.createStringLiteral(transformedModuleSpecifier)));
            }
            return undefined;
        }
    }
...
function transformSourceFile(sourceFile, context) {
    ...
    const visitorContext = (() => {
        ...
        const addImport = (declaration, noEmit = false) => {
        ...
        const getImportDeclarationWithModuleSpecifier = (moduleSpecifier) => [...imports.keys()].find(declaration => declaration.moduleSpecifier != null && typescript.isStringLiteralLike(declaration.moduleSpecifier) && declaration.moduleSpecifier.text === moduleSpecifier);
        ...
        !locals.has(identifier) &&
            ...
            !reservedWords.check(identifier, "es3", true) &&
            !reservedWords.check(identifier, "es5", true) &&
            !reservedWords.check(identifier, "es2015", true);
        ...
        return {
            ...
            getFreeIdentifier,
            ignoreIdentifier,
            getModuleExportsForPath: p => moduleExportsMap.get(path__default["default"].normalize(p)),
            addModuleExportsForPath: (p, exports) => moduleExportsMap.set(path__default["default"].normalize(p), exports),
            get imports() {
    ...
    visitorContext.addModuleExportsForPath(path__default["default"].normalize(sourceFile.fileName), moduleExports);
    if (!visitorContext.onlyExports && shouldDebug(visitorContext.debug, sourceFile) && visitorContext.printer != null) {
        visitorContext.logger.debug("===", path__default["default"].native.normalize(sourceFile.fileName), "===");
...
function cjsToEsmTransformer(options = {}) {
    return context => {
        ...
        const visitorContext = {
            ...
            factory: compatfactory.ensureNodeFactory((_a = context.factory) !== null && _a !== void 0 ? _a : typescript),
...
function createCompilerHost({ cwd, fileSystem, typescript }) {
    return {
        ...
        getSourceFile(fileName, languageVersion) {
            const normalized = path__default["default"].normalize(fileName);
        ...
        getCurrentDirectory() {
            return path__default["default"].native.normalize(cwd);
        },
        getDirectories(directoryName) {
            return typescript.sys.getDirectories(directoryName).map(path__default["default"].native.normalize);
        },
        ...
        realpath(p) {
            return path__default["default"].native.normalize(p);
        }
    };
}
/**
...
async function transformTask(options) {
    ...
    logger.debug("Options:", util.inspect({ input, outDir, cwd, write, debug, preserveModuleSpecifiers }, {
        colors: true,
        depth: Infinity,
        maxArrayLength: Infinity
    }));
    // Match files based on the glob(s)
    const matchedFiles = new Set(ensureArray(input).flatMap(glob => fastGlob.sync(path__default["default"].normalize(glob), { fs: fileSystem }).map(file => (path__default["default"].isAbsolute(file) ? path__default["default"].normalize(file) : path__default["default"].join(cwd, file)))));
    logger.debug(`Matched files:`, matchedFiles.size < 1 ? "(none)" : [...matchedFiles].map(f => `"${path__default["default"].native.normalize(f)}"`).join(", "));
    ...
    program.emit(undefined, (fileName, text) => {
        const nativeNormalizedFileName = path__default["default"].native.normalize(fileName);
        ...
        if (write) {
            fileSystem.mkdirSync(path__default["default"].native.dirname(nativeNormalizedFileName), { recursive: true });
            fileSystem.writeFileSync(nativeNormalizedFileName, text);
        }
        logger.info(`${chalk__default["default"].green("✔")} ${path__default["default"].native.relative(cwd, nativeNormalizedFileName)}`);
    }, undefined, false, cjsToEsm(options));
    return result;
}

function injectTransformCommand(options) {
    createCommand(options.program, {
        name: "transform",
...
function configureCommands({ keepAliveOnError = false, commandLoaders = [injectTransformCommand], args = [...process.argv], ...injectCommandOptions } = {}) {
