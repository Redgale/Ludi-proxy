...
import { sync as sync$1 } from 'fast-glob';
...
function resolvePath({ id, parent, cwd, prioritizedPackageKeys = ["es2015", "esm2015", "module", "jsnext:main", "main", "browser"], prioritizedExtensions = ["", ".js", ".mjs", ".jsx", ".ts", ".tsx", ".json"], moduleDirectory = "node_modules", fileSystem, resolveCache }) {
    ...
    try {
        const resolveResult = path.normalize(sync(id, {
...
function createTaskOptions({ typescript = TS, fileSystem = realReadonlyFileSystem, debug = false, cwd = process.cwd(), preserveModuleSpecifiers = "external", logger = new Logger(debug !== false ? 3 /* DEBUG */ : 0 /* NONE */) } = {}) {
    return {
        typescript,
        fileSystem,
        debug,
...
async function transformTask(options) {
    ...
    logger.debug("Options:", inspect({ input, outDir, cwd, write, debug, preserveModuleSpecifiers }, {
        colors: true,
        depth: Infinity,
        maxArrayLength: Infinity
    }));
    // Match files based on the glob(s)
    const matchedFiles = new Set(ensureArray(input).flatMap(glob => sync$1(path.normalize(glob), { fs: fileSystem }).map(file => (path.isAbsolute(file) ? path.normalize(file) : path.join(cwd, file)))));
